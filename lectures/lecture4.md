...
3. static_cast - наиболее часто используемый тип приведения. Оператор можно использовать для таких операций, как преобразование указателя на базовый класс в указатель на
производный класс, однако такие преобразования не всегда являются безопасными. Пример использования:
    ```c++
    int a = 7;
